...
public class ConST171Base : DeviceLink.DeviceBase.DeviceBase
{
    ...
    {
        return SendForResultAsync(Command.Read("SYSTem:MFR"), r => _codec.ExtractString(r), ct);
    }

    /// <summary>
    /// 写入设备生产厂家 —— SYSTem:MFR value
    /// </summary>
    public Task SetManufacturerAsync(string value, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("SYSTem:MFR", value), ct);
    }
    ...
    {
        return SendForResultAsync(Command.Read("SYSTem:MODel"), r => _codec.ExtractString(r), ct);
    }

    /// <summary>
    /// 写入设备型号 —— SYSTem:MODel value
    /// </summary>
    public Task SetModelAsync(string value, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("SYSTem:MODel", value), ct);
    }
    ...
    {
        return SendForResultAsync(Command.Read("SYSTem:SN"), r => _codec.ExtractString(r), ct);
    }

    /// <summary>
    /// 写入设备序列号 —— SYSTem:SN value
    /// </summary>
    public Task SetSerialNumberAsync(string value, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("SYSTem:SN", value), ct);
    }
    ...
    {
        return SendForResultAsync(Command.Read("SYSTem:VERSion"), ParseVersionInfo, ct);
    }
    ...
    {
        return SendForResultAsync(Command.Read("SYSTem:RS232:INFo"), ParseRs232Settings, ct);
    }

    /// <summary>
    /// 设置 MCU 与 PC 串口参数 —— SYSTem:RS232:INFo BaudRate,DataBits,StopBits,Parity
    /// </summary>
    ...
    /// <summary>
    /// 切换到主界面 —— SYSTem:HOMe（请确保设备处于空闲状态）
    /// </summary>
    ...
    {
        return SendNonQueryAsync(Command.Write("SYSTem:RESTart"), ct);
    }

    /// <summary>
    /// 恢复出厂设置 —— SYSTem:RESet password（默认密码 123456）
    /// </summary>
    public Task FactoryResetAsync(string password = "123456", CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("SYSTem:RESet", password), ct);
    }

    /// <summary>
    /// 查询系统声音启用状态 —— SYSTem:SOUNd?（true=启用）
    /// </summary>
    public Task<bool> GetSoundAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("SYSTem:SOUNd"), r => IsOne(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置系统声音启用状态 —— SYSTem:SOUNd value
    /// </summary>
    public Task SetSoundAsync(bool enable, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("SYSTem:SOUNd", enable ? "1" : "0"), ct);
    }

    /// <summary>
    /// 查询系统亮度 —— SYSTem:BRIG?（返回值 0-100）
    /// </summary>
    public Task<int> GetBrightnessAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("SYSTem:BRIG"), r => ParseInt(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置系统亮度 —— SYSTem:BRIG value（取值范围 0-100）
    /// </summary>
    public Task SetBrightnessAsync(int value, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("SYSTem:BRIG", value.ToString()), ct);
    }

    /// <summary>
    /// 查询当前语言 —— SYSTem:LANGuage?（如 zh-CN / en-US）
    /// </summary>
    ...
    {
        return SendForResultAsync(Command.Read("PRESsure"), ParseDualPressureValue, ct);
    }
    ...
    {
        return SendForResultAsync(Command.Read("PRESsure", module.ToScpiString()), ParsePressureValue, ct);
    }

    /// <summary>
    /// 获取指定气源控制状态 —— PRESsure:CONTrol? source（true=运行中）
    /// </summary>
    public Task<bool> GetPressureControlStateAsync(SourceModule module, CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("PRESsure:CONTrol", module.ToScpiString()), r => IsOne(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置指定气源控制状态 —— PRESsure:CONTrol source,state
    /// </summary>
    public Task SetPressureControlStateAsync(SourceModule module, bool running, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("PRESsure:CONTrol", module.ToScpiString(), running ? "1" : "0"), ct);
    }

    /// <summary>
    /// 获取指定气源压力单位 —— UNIT? source
    /// </summary>
    public Task<string> GetPressureUnitAsync(SourceModule module, CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("UNIT", module.ToScpiString()), r => _codec.ExtractString(r), ct);
    }

    /// <summary>
    /// 设置指定气源压力单位 —— UNIT source,unit
    /// </summary>
    public Task SetPressureUnitAsync(SourceModule module, string unit, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("UNIT", module.ToScpiString(), unit), ct);
    }

    /// <summary>
    /// 获取指定气源压力控制范围 —— PRESsure:RANGe? source（返回 下限:上限 kPa）
    /// </summary>
    public Task<PressureRange> GetPressureRangeAsync(SourceModule module, CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("PRESsure:RANGe", module.ToScpiString()), ParsePressureRange, ct);
    }

    /// <summary>
    /// 设置指定气源压力控制范围 —— PRESsure:RANGe source,min:max
    /// </summary>
    public Task SetPressureRangeAsync(SourceModule module, double min, double max, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("PRESsure:RANGe", module.ToScpiString(), $"{min:F2}:{max:F2}"), ct);
    }
    ...
    /// <summary>
    /// 读取指定泵原始压力值 —— PRESsure:RAW? source（内部指令）
    /// </summary>
    public Task<PressureValue> GetRawPressureAsync(SourceModule module, CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("PRESsure:RAW", module.ToScpiString()), ParsePressureValue, ct);
    }

    /// <summary>
    /// 获取正压气源静音模式 —— PRESsure:MUTE?（内部指令）
    /// </summary>
    public Task<bool> GetMuteAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("PRESsure:MUTE"), r => IsOne(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置正压气源静音模式 —— PRESsure:MUTE value（内部指令）
    /// </summary>
    public Task SetMuteAsync(bool enable, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("PRESsure:MUTE", enable ? "1" : "0"), ct);
    }

    /// <summary>
    /// 获取正压上限设定点模式 —— PRESsure:ADJ?（内部指令）
    /// </summary>
    public Task<bool> GetAdjAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("PRESsure:ADJ"), r => IsOne(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置正压上限设定点模式 —— PRESsure:ADJ value（内部指令）
    /// </summary>
    public Task SetAdjAsync(bool activeRelief, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("PRESsure:ADJ", activeRelief ? "1" : "0"), ct);
    }

    /// <summary>
    /// 获取真空气源开机排水模式 —— PRESsure:VACUum:VENT?（内部指令）
    /// </summary>
    public Task<bool> GetVacuumVentAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("PRESsure:VACUum:VENT"), r => IsOne(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置真空气源开机排水模式 —— PRESsure:VACUum:VENT value（内部指令）
    /// </summary>
    public Task SetVacuumVentAsync(bool enable, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("PRESsure:VACUum:VENT", enable ? "1" : "0"), ct);
    }
    ...
    {
        return SendForResultAsync(Command.Read("CALibration:DATA:VALue", module.ToScpiString(), password, type.ToString()), ParseCalibrationRecord, ct);
    }

    /// <summary>
    /// 写入指定气源传感器的校准数据 —— CALibration:DATA:VALue
    /// </summary>
    ...
    {
        return SendNonQueryAsync(Command.Write("CALibration:DATA:VALue", module.ToScpiString(), password, count.ToString(), points, values, year.ToString(), month.ToString(), day.ToString()), ct);
    }

    /// <summary>
    /// 重置校准数据（返回被重置前的校准记录）—— CALibration:DATA:RESet? ModuleID,password,type
    /// </summary>
    ...
    {
        return SendForResultAsync(Command.Read("CALibration:DATA:RESet", module.ToScpiString(), password, type.ToString()), ParseCalibrationRecord, ct);
    }

    /// <summary>
    /// 控制【增压/真空泵】进入校准状态 —— CALibration:METHod:STARt ModuleID（内部，不对外开放）
    /// </summary>
    public Task StartCalibrationAsync(SourceModule module, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("CALibration:METHod:STARt", module.ToScpiString()), ct);
    }

    /// <summary>
    /// 控制【增压/真空泵】退出校准状态 —— CALibration:METHod:STOP ModuleID（内部，不对外开放）
    /// </summary>
    public Task StopCalibrationAsync(SourceModule module, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("CALibration:METHod:STOP", module.ToScpiString()), ct);
    }

    /// <summary>
    /// 控制【增压/真空泵】达到目标压力值 —— CALibration:METHod:RUN ModuleID,value（内部，不对外开放）
    /// </summary>
    public Task RunCalibrationAsync(SourceModule module, double value, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("CALibration:METHod:RUN", module.ToScpiString(), value.ToString("F2", CultureInfo.InvariantCulture)), ct);
    }

    /// <summary>
    /// 更改设备 RS232 串口波特率 —— SYSTem:MCUBaudrate value（内部指令）
    /// </summary>
    public Task SetMcuBaudrateAsync(int baudRate, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("SYSTem:MCUBaudrate", baudRate.ToString()), ct);
    }

    /// <summary>
    /// 软重启指令（用于升级）—— DIAGnostic:SOFTreboot waitTimeMs
    /// </summary>
    public Task SoftRebootAsync(int waitTimeMs = -1, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:SOFTreboot", waitTimeMs.ToString()), ct);
    }

    /// <summary>
    /// 读取序列号（诊断）—— DIAGnostic:SN?
    /// </summary>
    public Task<string> GetDiagSerialNumberAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:SN"), r => _codec.ExtractString(r), ct);
    }

    /// <summary>
    /// 设置序列号（诊断）—— DIAGnostic:SN value
    /// </summary>
    public Task SetDiagSerialNumberAsync(string value, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:SN", value), ct);
    }

    /// <summary>
    /// 读取型号（诊断）—— DIAGnostic:MODel?
    /// </summary>
    ...
    /// <summary>
    /// 设置型号（诊断）—— DIAGnostic:MODel value
    /// </summary>
    public Task SetDiagModelAsync(string value, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:MODel", value), ct);
    }

    /// <summary>
    /// 读取生产厂家（诊断）—— DIAGnostic:MFR?
    /// </summary>
    public Task<string> GetDiagManufacturerAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:MFR"), r => _codec.ExtractString(r), ct);
    }

    /// <summary>
    /// 设置生产厂家（诊断）—— DIAGnostic:MFR value
    /// </summary>
    public Task SetDiagManufacturerAsync(string value, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:MFR", value), ct);
    }

    /// <summary>
    /// 读取生产日期 —— DIAGnostic:MFRDate?（如 2026.1.1）
    /// </summary>
    public Task<string> GetManufactureDateAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:MFRDate"), r => _codec.ExtractString(r), ct);
    }

    /// <summary>
    /// 设置生产日期 —— DIAGnostic:MFRDate value（年月日以.分隔）
    /// </summary>
    public Task SetManufactureDateAsync(string date, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:MFRDate", date), ct);
    }

    /// <summary>
    /// 读取开机 LOGO —— DIAGnostic:LOGO?（0=ConST, 1=Additel, 2=定制）
    /// </summary>
    public Task<int> GetLogoAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:LOGO"), r => ParseInt(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置开机 LOGO —— DIAGnostic:LOGO value
    /// </summary>
    public Task SetLogoAsync(int value, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:LOGO", value.ToString()), ct);
    }

    /// <summary>
    /// 重启 LCD 显示模块 —— DIAGnostic:LCD:REBoot（解决屏幕卡死或显示异常）
    /// </summary>
    public Task RebootLcdAsync(CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:LCD:REBoot"), ct);
    }

    /// <summary>
    /// LCD 握手 —— DIAGnostic:LCD:HANDshake（检测通信链路是否正常）
    /// </summary>
    public Task LcdHandshakeAsync(CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:LCD:HANDshake"), ct);
    }
    ...
    {
        return SendForResultAsync(Command.Read("DIAGnostic:FAN", module.ToScpiString()), r => ParseInt(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置风扇转速 —— DIAGnostic:FAN name,pwm（pwm 范围 0~1）
    /// </summary>
    public Task SetFanSpeedAsync(SourceModule module, double pwm, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:FAN", module.ToScpiString(), pwm.ToString("F2", CultureInfo.InvariantCulture)), ct);
    }

    /// <summary>
    /// 读取常开（泄压阀）控制指令状态 —— DIAGnostic:VENT?（true=打开）
    /// </summary>
    public Task<bool> GetVentAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:VENT"), r => IsOne(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置常开（泄压阀）控制指令状态 —— DIAGnostic:VENT val
    /// </summary>
    public Task SetVentAsync(bool open, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:VENT", open ? "1" : "0"), ct);
    }

    /// <summary>
    /// 读取阀门状态 —— DIAGnostic:VALVe? name（valveId 1~4）
    /// </summary>
    public Task<bool> GetValveAsync(int valveId, CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:VALVe", valveId.ToString()), r => IsOne(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 设置阀门开关状态 —— DIAGnostic:VALVe name,val
    /// </summary>
    public Task SetValveAsync(int valveId, bool open, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:VALVe", valveId.ToString(), open ? "1" : "0"), ct);
    }

    /// <summary>
    /// 查询主板温度 —— DIAGnostic:BOARd:TEMP?（返回 ℃）
    /// </summary>
    public Task<double> GetBoardTemperatureAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:BOARd:TEMP"), r =>
        {
    ...
    /// <summary>
    /// 查询主板电压 —— DIAGnostic:BOARd:VOLTage?（24V, Boost传感器, Vacuum传感器）
    /// </summary>
    public Task<BoardVoltage> GetBoardVoltageAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:BOARd:VOLTage"), ParseBoardVoltage, ct);
    }

    /// <summary>
    /// 查询泵运行状态 —— DIAGnostic:PUMP? name（true=运行中）
    /// </summary>
    public Task<bool> GetPumpStateAsync(SourceModule module, CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:PUMP", module.ToScpiString()), r => IsOne(_codec.ExtractString(r)), ct);
    }
    ...
    {
        return SendNonQueryAsync(Command.Write("DIAGnostic:PUMP", module.ToScpiString(), running ? "1" : "0"), ct);
    }

    /// <summary>
    /// 查询 FOC 状态 —— DIAGnostic:FOC?（前级FOC和增压FOC正常/错误）
    /// </summary>
    public Task<FocState> GetFocStateAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:FOC"), ParseFocState, ct);
    }

    /// <summary>
    /// 查询泵温度 —— DIAGnostic:PUMP:TEMP?（前级泵温度, 增压泵温度 ℃）
    /// </summary>
    public Task<PumpTemperatures> GetPumpTemperatureAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:PUMP:TEMP"), ParsePumpTemperatures, ct);
    }

    /// <summary>
    /// 查询泵电流 —— DIAGnostic:PUMP:CURRent?（前级泵电流, 增压泵电流 A）
    /// </summary>
    public Task<PumpCurrents> GetPumpCurrentAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("DIAGnostic:PUMP:CURRent"), ParsePumpCurrents, ct);
    }
    ...
    {
        return SendNonQueryAsync(Command.Write("TEST:FUNCtion", enter ? "1" : "0"), ct);
    }

    /// <summary>
    /// 查询整机测试状态 —— TEST:FUNCtion?（true=正在进行测试）
    /// </summary>
    public Task<bool> GetTestModeAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("TEST:FUNCtion"), r => IsOne(_codec.ExtractString(r)), ct);
    }
    ...
    {
        return SendNonQueryAsync(Command.Write("TEST:BEEP", enable ? "1" : "0"), ct);
    }

    /// <summary>
    /// 启动屏幕测试 —— TEST:SCReen item（0=坏点,1=触摸,2=亮度,3=蜂鸣器）
    /// </summary>
    public Task StartScreenTestAsync(int item, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("TEST:SCReen", item.ToString()), ct);
    }

    /// <summary>
    /// 读取屏幕测试结果 —— TEST:SCReen:RESUlt? item（0=未进行,1=进行中,2=失败,3=通过）
    /// </summary>
    public Task<int> GetScreenTestResultAsync(int item, CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("TEST:SCReen:RESUlt", item.ToString()), r => ParseInt(_codec.ExtractString(r)), ct);
    }

    /// <summary>
    /// 读取吹扫测试状态 —— TEST:BLOW?（0=未进行,1=进行中,2=吹扫结束）
    /// </summary>
    public Task<int> GetBlowTestStateAsync(CancellationToken ct = default)
    {
        return SendForResultAsync(Command.Read("TEST:BLOW"), r => ParseInt(_codec.ExtractString(r)), ct);
    }
    ...
    {
        return SendNonQueryAsync(Command.Write("TEST:BLOW", start ? "1" : "0"), ct);
    }
    ...
    {
        return SendNonQueryAsync(Command.Write("TEST:DST:TOUCh"), ct);
    }
    ...
    {
        return SendNonQueryAsync(Command.Write("TEST:DST:REFResh"), ct);
    }

    /// <summary>
    /// 设置测试屏幕要刷新的数据 —— TEST:REFResh data（uint32 范围内）
    /// </summary>
    public Task SetRefreshTestDataAsync(uint data, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("TEST:REFResh", data.ToString()), ct);
    }

    /// <summary>
    /// 建立 OTA 连接 —— OTAUpdate:CONNect base64Config
    /// </summary>
    public Task OtaConnectAsync(string base64Config, CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("OTAUpdate:CONNect", base64Config), ct);
    }
    ...
    {
        return SendNonQueryAsync(Command.Write("OTAUpdate:TRANs", sequenceNumber.ToString(), data), ct);
    }

    /// <summary>
    /// 执行升级 —— OTAUpdate:UPDAte（传输完成后触发固件校验升级）
    /// </summary>
    public Task OtaUpdateAsync(CancellationToken ct = default)
    {
        return SendNonQueryAsync(Command.Write("OTAUpdate:UPDAte"), ct);
    }
