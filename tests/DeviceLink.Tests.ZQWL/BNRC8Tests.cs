...
namespace DeviceLink.Tests.ZQWL
{
    public class BNRC8Tests
    {
        ...
        [Fact]
        public async Task BNRC8_IsExistAsync_VersionContainsBNRC08_ShouldReturnTrue()
        {
            var mockSession = new Mock<ISession>();
            // 版本号格式：1BNRC08
            ...
            var response = new byte[] { 0x01, 0x66,
                                        0x31, 0x42, 0x4E, 0x52, 0x43, 0x30, 0x38, 0x00 };
            mockSession.Setup(s => s.SendAndReceiveAsync(It.IsAny<byte[]>(), It.IsAny<CancellationToken>()))
                       .ReturnsAsync(response);
        ...
        [Fact]
        public async Task BNRC8_IsExistAsync_VersionContainsIO_ShouldReturnFalse()
        {
            var mockSession = new Mock<ISession>();
            // 版本号格式：IO-04-00（不包含 BNRC08）
            var response = new byte[] { 0x01, 0x66, 0x00, 0x00,
                                        0x49, 0x4F, 0x2D, 0x30, 0x34, 0x2D, 0x30, 0x30, 0x00 };
            mockSession.Setup(s => s.SendAndReceiveAsync(It.IsAny<byte[]>(), It.IsAny<CancellationToken>()))
                       .ReturnsAsync(response);
        ...
        [Fact]
        public async Task BNRC8_IsExistAsync_VersionContainsBN_ShouldReturnFalse()
        {
            var mockSession = new Mock<ISession>();
            // 版本号格式：BN-08-V1（不包含 BNRC08）
            var response = new byte[] { 0x01, 0x66, 0x00, 0x00,
