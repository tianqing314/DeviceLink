...
namespace DeviceLink.Core.Framing
{
    ...
    public class DelimiterFrameStrategy : IFrameStrategy
    {
        ...
        public DelimiterFrameStrategy(string delimiter, int minDelimiterCount = 1)
            : this(System.Text.Encoding.UTF8.GetBytes(delimiter), minDelimiterCount)
        { }
        ...
        public bool TryMatchFrame(byte[] accumulated, out int frameLength)
        {
            ...
            while (pos <= accumulated.Length - _delimiter.Length)
            {
                ...
                {
                    matchCount++;
                    lastMatchEnd = pos + _delimiter.Length;
